...
fileName = 'FinalData_6OHDA_H.h5'; %to be changed by user
includeSkip = false;
packTable = 'h5_Final_H_skip';
...
%Sess = [{'2976_BaselineS'},{'1253_BaselineS'},{'0761_BaselineA'}]%,{'1253_BaselineS'}]
...
for s = 1:numel(Sess)
    ...
    if addCa
        try 
           ...
           % add rawTraces: 
                ...
                % write attributes
                h5writeatt(fileName,['/',mouseNum,'/',session,'/traces/dff'],'description','Raw traces. sorted by td-tomato labed cells first')
                h5writeatt(fileName,['/',mouseNum,'/',session,'/traces/dff'],'Post',Post)
                h5writeatt(fileName,['/',mouseNum,'/',session,'/traces/dff'],'Pre',Pre)
                h5writeatt(fileName,['/',mouseNum,'/',session,'/traces/dff'],'numMSN',numN-numTD)
                h5writeatt(fileName,['/',mouseNum,'/',session,'/traces/dff'],'creType',type)
    ...
    if addMvmt
        try
            ...
            update(conn,'data_final','perfectAlign_mvmt',{perfctAlign},sprintf("Where Suffix= '%s'",suffix))
            
            if suffix == "1222_day15A"
               startF = find(tiffTS.tiffTs ==0,1,'first');
               tiffTS.tiffTs = tiffTS.tiffTs(startF:end);
            end
            
            if tiffTS.tiffTs(1) > 1 && tiffTS.tiffTs(1) < 11000 && isempty(find(tiffTS.tiffTs ==0))
                [~, startT] = min(abs(mvmt.t-tiffTS.tiffTs(1)));
                mvmt.t = mvmt.t(startT:end)-mvmt.t(startT);
                mvmt.phi = mvmt.phi(startT:end);
                mvmt.rotation = mvmt.rotation(startT:end);
                mvmt.speed = mvmt.speed(startT:end);
            end
    ...
    if addEphys
        try
            if suffix == "1208_day12" || suffix == "2976_day4"
                continue
            end
            ...
            tiffTS = tiffTS.tiffTs;
            try    
                perfctAlign = uint8(max(size(lfpData.Frames))== max(size(tiffTS)));
            catch er
                perfctAlign = 0;
            end 
       
            if tiffTS(1) > 1 && isempty(find(tiffTS ==0))
                [~,sFrame] = min(abs(lfpData.Frames - lfpData.Frames(1)-tiffTS(1)));
            else
                sFrame = 1;
            end


            update(conn,'data_final','perfectAlign_lfp',{perfctAlign},sprintf("Where Suffix= '%s'",suffix))
            
            lfp = lfpData.lfp10;
            if sFrame < 11000               
                lfp = lfp(lfpData.FramesTS(sFrame):end); 
            else
