...
for f=1:size(pathways,1)
    ...
    %vidList = dir(['Z:\Data\DanaTemp\PreProcessing\green\m_n_',suffix,'*']);
    %data = [];
    %for vid=1:numel(vidList)
        %file = [vidList(vid).folder,'\',vidList(vid).name];
        file = [pathways(f).folder,'\',pathways(f).name];
    ...
    %end
    %Ired = max(data,[],3);
    save([savedir,'Ired_m_',suffix],'Ired')%'_day',num2str(pathways(f).day)],'I')
    cd Z:\Data\DanaTemp\
end