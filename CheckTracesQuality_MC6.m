...
%    r_ori = load([tracedir_ori,tracePrefix_ori,suffix,'.mat']);
%    r_ori = r_ori.r_out;
    ...
    if ~isfield(r_out,'perimeter') 
        if ~isfield(r_ori,'perimeter')
            continue
        end
        [r_out(:).perimeter] = r_ori(:).perimeter;
    end
    if ~isfield(r_outH,'perimeter')
        [r_outH(:).perimeter] = r_ori(:).perimeter;
    end
    
    if isfield(r_out,'skip')
     disp('did this yesterday')
    ...
    close all
     ...
     if isfield(r_ori,'TD_H')
        [r_outH(:).TD_H] = deal(0);
        [r_outH(:).TD_H] = r_ori(:).TD_H;
        [r_out(:).TD_H] = deal(0);
        [r_out(:).TD_H] = r_ori(:).TD_H;
     end
