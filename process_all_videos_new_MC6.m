...
savedir = '/home/dana_z/HD2/traces_MC6/';
...
conn = database('preProcess','auto_processing','dz_preProcess', ...
    'Vendor','MySQL', ...
    'Server','localhost');
skipSess =  table2array(select(conn, 'Select Suffix from data_final where MC6=1 or skip=1;'));
...
for s = 1:numel(Sess)
   ...
   clear r greens
    ...
    motion_correction_6OHDA_MC6(filename, suffix, savedir,R)
    cd /home/dana_z/ssd_2TB/PreProcessing
    t = datetime('now');
    DateString = datestr(t);
    update(conn,'data_final',{'MC6','date_MC6'},...
