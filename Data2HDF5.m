...
fileName = 'Data_6OHDA.h5'; %to be changed by user

conn = database('preProcess','auto_processing','dz_preProcess', ...
    'Vendor','MySQL', ...
    'Server','localhost');

Sess =  table2array(select(conn, 'Select Suffix from data where hdf5=0;'));
...
for s = 1:numel(Sess)
    ...
    h5Stat = table2array(select(conn,sprintf("Select mvmt,lfp,Ca  from h5_Prog where Suffix = '%s';",suffix)));
    ...
    if addCa
        try 
            ...
            [~,I] = sort(arrayfun (@(x) x.TD, r_out),'descend');
            r_out = r_out(I);
            clear I
            numTD = sum([r_out(:).TD]);
       ...
       try
           datainsert(conn,'h5_Prog',{'Ca','lfp','mvmt','Suffix'},...
                {1,0,0,suffix}) 
       catch er
            update(conn,'h5_Prog','Ca',1,sprintf("Where Suffix= '%s'",suffix))
       end
        ...
        try
           ...
           try
               datainsert(conn,'h5_Prog',{'Ca','lfp','mvmt','Suffix'},...
                ...
                update(conn,'h5_Prog','mvmt',1,sprintf("Where Suffix= '%s'",suffix))
           end
    ...
    if addEphys
        try
            ...
            lfpData = load([mvmtdir,'ePhys_',suffix,'.mat']);
            try
                tiffTS = load([TSdir,'tiffTs_',suffix,'.mat']);
           ...
           try
               datainsert(conn,'h5_Prog',{'Ca','lfp','mvmt','Suffix'},...
                ...
                update(conn,'h5_Prog','lfp',1,sprintf("Where Suffix= '%s'",suffix))
           end
    ...
    clearvars -except tracedir TSdir mvmtdir ephysdir fileName conn Sess fid s
end
%% close file
H5F.close(fid);
