function motion_correction_6OHDA_MC6(filename, suffix, savedir,R) %modified by michael romano and then Dana Zemel
    ...
    for n = nFiles:-1:1
        ...
        % ------------------------------------------------------------------------------------------
         ...
         if n == nFiles
             ...
             [data_m_f, ~] = apply_correctMotion(data_m_f, sh);
             [data_m, ~] = apply_correctMotion(data, sh);
             
%               fprintf(['Saving temp',short_fname{n},'\n']);
%               save_filename = ['/home/dana_z/HD1/Homomorphic tests/m_f_',suffix,'_',num2str(n)];
%               matrix2tiff(data_m_f, save_filename, 'w');
             
             [data_m_f, procstart_mf.norm] = normalizeDataD(data_m_f);
             [data_m, procstart_m.norm] = normalizeDataD(data_m);
             r_out=extract_traceMC2(R, data_m);
             r_out_f=extract_traceMC2(R, data_m_f);
         else
...
%              matrix2tiff(data_m_f, save_filename, 'w');
             ...
             [data_m_f, procstart_mf.norm] = normalizeDataD(data_m_f, procstart_mf.norm);
             [data_m, procstart_m.norm] = normalizeDataD(data_m, procstart_m.norm);
             r_temp_f=extract_traceMC2(R,data_m_f);
             r_temp=extract_traceMC2(R,data_m);
             for r = 1:numel(R)
                r_out(r).trace = cat(2,r_temp(r).trace,r_out(r).trace);
                r_out_f(r).trace = cat(2,r_temp_f(r).trace,r_out_f(r).trace);
             end
         end
         
%          fprintf(['Saving ',short_fname{n},'\n']);
%          save_filename = ['/home/dana_z/HD1/Homomorphic tests/m_f_n_',suffix,'_',num2str(n)];
%          matrix2tiff(data_m_f, save_filename, 'w');
