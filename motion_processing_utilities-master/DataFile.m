classdef DataFile < handle
   ...
   methods % Functions for Saving
	  ...
	  function closeFile(obj) % call at the end of a trial
		 if isopen(obj)
			tic
			while toc < .25%TODO: what is the deal???
			   if obj.iswriting
				  % 												pause(.01);
   ...
   methods (Static)
	  ...
	  function datafileObj = recoverDataFile(protoObj)		 
		 ...
		 % 		 dataFileObj = DataFile.empty(numel(tphName),0);
		 ...
		 for kFile = 1:numel(tphName)
			...
			% OR INFO-FILE
			% 			infldBytes = 0;
			% 			for kField = 1:size(protoObj.infoFormat,1)
			% 			   % 			   infldClass = protoObj.infoFormat{kField,1};
			% 			   % 			   infldSize = protoObj.infoFormat{kField,2};
			% 			   % 			   infldProto = zeros(infldSize, 'double');
			% 			   % 			   m = whos('infldProto');
			% 			   infldBytes = infldBytes + 64;
			% 			end
			% 			bytesPerFrame = infldBytes;
			% 			[fid, msg] = fopen(fullfile(propStruct.rootPath,'FrameSyncFiles', propStruct.infoFileName), 'rb');
			% 			if fid < 1
			% 			   dataFileObj(kFile,1) = [];
			% 			   warning(msg)
			% 			   continue
			...
			% 			nBytes = ftell(fid);
			% 			fclose(fid);
			% 			numFramesFromInfo = nBytes / bytesPerFrame;
